...
WEBAPP_URL = "https://police-bot-xxxx.onrender.com"  # ЗАМЕНИ НА СВОЙ URL
...
def api_new_game():
    ...
    diff = data.get('difficulty', 'major')
    d = DIFFICULTY[diff]
    state = {
...
def api_end_day():
    ...
    if random.random() < 0.4:
        ...
        if "поджог" in night_event.lower() or "погром" in night_event.lower():
            state['stability'] = max(0, state['stability'] - random.randint(1,5))
            state['riots'][random.choice(DISTRICTS)] = min(100, state['riots'][random.choice(DISTRICTS)] + random.randint(1,10))
        elif "информатор" in night_event.lower():
            state['threat'] = max(0, state['threat'] - random.randint(1,5))
    game_over_msg = None
...
def api_extra_action():
    ...
    if action_type == "patrol":
        if not [s for s in state['squads'] if s['location'] == district]: return jsonify({"error": "Нет отрядов в районе."})
        state['riots'][district] = max(0, state['riots'][district] - random.randint(3,8))
        state['budget'] += 300000; result_msg = f"👮 Патруль в {district}. +300 000 ₽"
        ...
        if strength >= riot / 2:
            state['riots'][district] = max(0, riot - random.randint(25,40))
            state['stability'] = min(100, state['stability'] + random.randint(3,8))
            state['budget'] += random.randint(500000, 2000000); result_msg = f"🚨 Бунт подавлен!"
        else:
            state['riots'][district] = min(100, riot + random.randint(5,15))
            for s in squads: s['strength'] = max(1, s['strength'] - random.randint(1,3))
            result_msg = "❌ Сил не хватило!"
    elif action_type == "hq":
        ...
        result_msg = f"🏴 Штаб в {district} на 5 дней."
        ...
        if state['curfew']:
            ...
            state['trust'] = max(0, state['trust'] - 10); result_msg = "🌙 Комендантский час введён."
        ...
        state['budget'] -= 500000; result_msg = f"🕵️ {generate_sync('Дай разведсводку (1 предложение).')}"
    ...
    save_state(user_id, state)
    return jsonify({"result": result_msg, "budget": state['budget'], "riots": state['riots'], "stability": state['stability'], "trust": state['trust'], "curfew": state.get('curfew',False), "hq": state.get('hq',{})})
...
def api_squad_action():
    ...
    if cmd == "form":
        ...
        cost = costs.get(t,2000000)
        if state['budget'] < cost: return jsonify({"error": f"Нужно {cost:,} ₽."})
        state['budget'] -= cost; name = f"Отряд-{len(state['squads'])+1}"
        state['squads'].append({"name":name,"location":"Центр","strength":10 if t=="спецназ" else 5,"morale":70,"type":t})
        result_msg = f"✅ {t} «{name}» создан."
    elif cmd == "move":
        for s in state['squads']:
            if s['name'].lower() == params.get('name','').lower():
                s['location'] = params.get('district','Центр'); result_msg = f"🚔 {s['name']} → {s['location']}"; break
    elif cmd == "reinforce":
        if state['budget'] < 1500000: return jsonify({"er
