...
def api_new_game():
    ...
    state = {
        ...
        "curfew": False, "alert_level": "green", "difficulty": diff,
        ...
        "police_stations": {}, "checkpoints": {}, "special_units": {}
    }
...
def api_start_day():
    ...
    station_income = sum(st.get('income', 0) for st in state.get('police_stations', {}).values() if st.get('active'))
    state['budget'] += patrol_income + station_income

    alert = state.get('alert_level', 'green')
    if alert == 'yellow':
        for r in DISTRICTS:
            if not state.get('curfew'): state['riots'][r] = min(100, state['riots'][r] + max(0, random.randint(1, 3) - 1))
        state['trust'] = max(0, state['trust'] - 2)
        state['budget'] += int(patrol_income * 0.1)
    elif alert == 'red':
        for r in DISTRICTS: state['riots'][r] = max(0, state['riots'][r] - random.randint(1, 5))
        state['trust'] = max(0, state['trust'] - 5)
        state['influence'] = max(0, state['influence'] - 3)
        state['corruption'] = min(100, state.get('corruption', 20) + random.randint(1, 3))
        state['army_available_always'] = True
    else:
        state['army_available_always'] = False

    for r in DISTRICTS:
        if state.get('police_stations', {}).get(r, {}).get('active'):
            state['riots'][r] = max(0, state['riots'][r] - 5)
    ...
    save_state(user_id, state)
    return jsonify({"summary": summary, "actions": actions, "day": state['day'], "budget": state['budget'], "income": patrol_income + station_income})
...
def api_extra_action():
    ...
    if action_type == "patrol": state['riots'][district] = max(0, state['riots'][district] - random.randint(3,8)); state['budget'] += 300000; result_msg = "👮 Патруль."
        ...
        if state.get('army_uses',0) <= 0 or (state['threat'] < 75 and not state.get('army_available_always')): return jsonify({"error": "Недоступно."})
        ...
        if 'checkpoints' not in state: state['checkpoints'] = {}
        state['checkpoints'][district] = {"active": True}; result_msg = "🚧 Блокпост установлен."
    save_state(user_id, state)
...
def api_squad_action():
    ...
    result_msg = ""
    if cmd == "form":
        t = params.get('type','патруль'); district = params.get('district', 'Центр')
        costs = {"патруль":2000000, "спецназ":3500000}
        if state['budget'] < costs.get(t,2000000): return jsonify({"error": "Нужно 2 000 000 ₽."})
        state['budget'] -= costs[t]; name = f"Отряд-{len(state['squads'])+1}"
        state['squads'].append({"name":name, "location": district, "strength":5, "morale":60, "type":t}); result_msg = f"✅ {t} «{name}» в {district}"
    elif cmd == "move":
...
@app.route('/api/buy_station', methods=['POST'])
def api_buy_station():
    data = request.json; user_id = data['user_id']; district = data.get('district', 'Центр')
    state = get_state(user_id)
    if not state: return jsonify({"error": "no game"})
    cost = 5000000
    if state['budget'] < cost: return jsonify({"error": f"Недостаточно средств. Требуется {cost:,} ₽."})
    if 'police_stations' not in state: state['police_stations'] = {}
    if state['police_stations'].get(district, {}).get('active'): return jsonify({"error": "Участок уже построен."})
    state['budget'] -= cost
    state['police_stations'][district] = {"active": True, "income": 200000}
    save_state(user_id, state)
    return jsonify({"budget": state['budget'], "police_stations": state['police_stations']})

@app.route('/api/move_unit', methods=['POST'])
def api_move_unit():
    data = request.json; user_id = data['user_id']; unit_type = data.get('unit_type'); district = data.get('district', 'Центр')
    state = get_state(user_id)
    if not state: return jsonify({"error": "no game"})
    if 'special_units' not in state or unit_type not in state['special_units']:
        return jsonify({"error": "Подразделение не создано"})
    state['special_units'][unit_type]['location'] = district
    save_state(user_id, state)
    return jsonify({"special_units": state['special_units']})

@app.route('/api/set_alert_level', methods=['POST'])
def api_set_alert_level():
    data = request.json; user_id = data['user_id']; level = data.get('level', 'green')
    if level not in ('green', 'yellow', 'red'): return jsonify({"error": "Неверный уровень"})
    state = get_state(user_id)
    if not state: return jsonify({"error": "no game"})
    if state.get('last_alert_change') == state['day']:
        return jsonify({"error": "Нельзя менять уровень чаще одного раза в день"})
    state['alert_level'] = level
    state['last_alert_change'] = state['day']
    save_state(user_id, state)
    return jsonify({"alert_level": level, "budget": state['budget'], "trust": state['trust'], "influence": state['influence']})
...
def flask_webhook():
    ...
    future = asyncio.run_coroutine_threadsafe(
        dp.feed_webhook_update(bot, request.get_json()),
        main_loop
    )
    future.result()
